...
class ViirsService(interface.implements(TopographyService)):
    ...
    def __init__(self, datatype, path_string, geo_directory = None, data_directory = None):
        ...
        if geo_directory is not None and data_directory is not None:
            self.add_directory(geo_directory, data_directory)

    def add_directory(self, geo_directory, data_directory):
    ...
    def __get_frame__(self, tile, query_bounds):
        ...
        df = self.__to_local__(df, query_bounds)
        return self.__get_json__(df)

    def __to_grid__(self, tile, query):
        variable_dataframe = tile.get_variable_dataframe(self.datatype, self.path_string)
        lat_dataframe = tile.get_geo_dataframe("Latitude")
        long_dataframe = tile.get_geo_dataframe("Longitude")
    ...
    def __to_local__(self, df, query):
        ...
        df["x"] = df["long"].apply(lambda x: x * long_quotient)
        df["y"] = df["lat"].apply(lambda y: y * lat_quotient)
